...
class TraineeController {
  ...
  list = async (req: IRequest, res: Response) => {
    try {
      ...
      if (result.length !== 0) {
        this.systemResponse.success(req, res, 'list of users', 200, { count: result.length, result});
      } else {
        this.systemResponse.failure(req, res, 'No user exits', 200, { count: result.length, result});
      }
    } catch (err) {
      this.systemResponse.failure(req, res, 'No user exits', 200, err);
    }
